...
- Default state (object in this case) contains `people` property
...
- The `useReducer()` hook takes two things.
  - The `reducer()` function which manipulate the `state` value
  - The `defaultState`
- Returns an array which contains
  - The `state` - whole state
  - The `dispatch()` function - the `state` using taking `action`
- Convension:
